                ...
                <div key={`chart-${acc.id}`} className={`flex flex-col items-center justify-end h-full group relative ${sortedAccounts.length <= 4 ? 'w-[80px] md:w-[100px]' : 'flex-1 min-w-[56px] max-w-[64px]'}`}>
                  ...
                  <div className="w-full relative flex justify-center items-end" style={{ height: `${normalizedHeight}%` }}>
                    ...
                    <div className="absolute bottom-full mb-3 w-full flex justify-center">
                      <span 
                        className={`font-data-mono text-[11px] sm:text-xs text-slate-200 group-hover:text-primary transition-colors tracking-tight whitespace-nowrap ${sortedAccounts.length <= 4 ? 'mb-2 text-sm' : ''}`} 
          ...
          {equityAccounts.map((acc, index) => (
            ...
            >
              ...
              <div className="absolute top-4 right-4 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-auto">
                {/* Move Category Icon */}
                <button 
                  onClick={() => handleMoveCategory(acc.id, 'Savings')}
