...
export default function Investments({ financialData }) {
  const { accounts, setAccounts, totalInvestasi } = financialData;
  ...
  const handleAddNewAsset = () => {
    ...
    const schemes = [
      { color: 'text-emerald-400', bg: 'bg-emerald-500/20', solidBg: 'bg-emerald-500', border: 'border-emerald-500/30' },
      ...
      { color: 'text-emerald-300', bg: 'bg-emerald-500-fixed/20', solidBg: 'bg-emerald-500-fixed', border: 'border-emerald-500-fixed/30' },
    ];
  ...
  const totalValue = totalInvestasi;

  // Sort accounts for the chart from highest to lowest
  const sortedAccounts = [...equityAccounts].sort((a, b) => b.value - a.value);

  return (
    <>
      <main className="md:ml-64 pt-24 px-4 md:px-margin-page pb-12 relative z-10 w-full md:w-[calc(100%-16rem)] min-h-screen">
        {/* Header Section */}
        <div className="mb-stack-lg">
          <h2 className="font-display-lg text-display-lg text-slate-200 mb-stack-sm tracking-tight">Investasi & Tabungan</h2>
          <p className="font-body-base text-body-base text-slate-400 max-w-2xl">
            Kelola alokasi aset manual Anda di berbagai sekuritas dan akun. Perbarui nilai nominal untuk menjaga keakuratan perhitungan total kekayaan.
          </p>
        </div>
        ...
        <div className="glass-card rounded-xl p-container-padding mb-8 relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 relative z-10 gap-4">
            <h3 className="font-headline-md text-headline-md text-slate-200 flex items-center gap-2">
              ...
              Alokasi Dompet Investasi
            </h3>
            <div className="sm:text-right bg-slate-800/50 px-4 py-2 rounded-lg border border-white/10">
              <p className="font-label-sm text-label-sm text-slate-400">Total Alokasi Terkini</p>
          ...
          <div className={`relative z-10 h-[380px] flex items-end ${sortedAccounts.length <= 4 ? 'justify-center gap-16 md:gap-32' : 'justify-between gap-4 md:gap-6'} overflow-x-auto pb-6 pt-32 scrollbar-hide px-4`}>
            {sortedAccounts.map((acc) => {
              ...
              const maxPercentage = Math.max(...equityAccounts.map(a => totalValue > 0 ? (a.value / totalValue) * 100 : 0));
                ...
                <div key={`chart-${acc.id}`} className={`flex flex-col items-center justify-end h-full group relative ${sortedAccounts.length <= 4 ? 'w-[80px] md:w-[100px]' : 'flex-1 min-w-[56px] max-w-[64px]'}`}>
                  ...
                  <div className="w-full relative flex justify-center items-end" style={{ height: `${normalizedHeight}%` }}>
                    ...
                    <div className="absolute bottom-full mb-3 w-full flex justify-center">
                      <span 
                        className={`font-data-mono text-[11px] sm:text-xs text-slate-200 group-hover:text-emerald-400 transition-colors tracking-tight whitespace-nowrap ${sortedAccounts.length <= 4 ? 'mb-2 text-sm' : ''}`} 
                  ...
                  <div className="mt-4 flex flex-col items-center justify-start h-10 w-full px-1 group-hover:-translate-y-1 transition-transform">
                    {nameParts.map((word, idx) => (
                      <span key={idx} className={`font-label-sm text-[11px] sm:text-[12px] text-center leading-tight truncate w-full ${idx === 0 ? 'text-slate-200 font-semibold' : 'text-slate-400'}`}>
                        {word}
                      </span>
                    ))}
                  </div>
                  <span className="font-data-mono text-[10px] text-emerald-400/70 mt-1 font-bold">{percentage}%</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {equityAccounts.map((acc, index) => (
            ...
            >
              ...
              {/* Drag Handle Icon */}
              <div className="absolute top-4 right-4 w-6 h-6 flex items-center justify-center text-slate-400/50 group-hover:text-slate-400 transition-colors pointer-events-none cursor-grab active:cursor-grabbing">
                    ...
                    {editingId === acc.id ? (
                      <div className="flex items-center gap-2">
                        <input 
                          ...
                          className="bg-surface-container-lowest border border-emerald-500 text-slate-200 font-headline-md text-headline-md rounded px-2 py-0.5 w-32 focus:outline-none"
                        />
                      </div>
                    ) : (
                      <div className="flex items-center gap-2 group/edit cursor-pointer" onClick={() => handleStartEdit(acc.id, acc.name)}>
                        ...
                        <span className="material-symbols-outlined text-[14px] text-slate-400/0 group-hover/edit:text-slate-400 transition-colors">edit</span>
                      </div>
                    )}
                    <span className="font-label-sm text-label-sm text-slate-400 uppercase tracking-wider">{acc.type}</span>
                  </div>
                </div>
              ...
              <div className="relative mt-stack-lg">
                <label className="font-label-sm text-label-sm text-slate-400 block mb-2">Nilai Portofolio</label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 font-data-mono text-data-mono text-slate-400">Rp</span>
                  <input 
                    className="w-full bg-surface-container-lowest/50 border border-outline-variant text-emerald-300 font-data-mono text-data-mono focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-lg py-2.5 pl-10 pr-3 transition-all text-right shadow-inner" 
                    type="text" 
              ...
              <div className="mt-stack-sm flex items-center justify-between">
                <span className="font-label-sm text-label-sm text-slate-400/70 flex items-center gap-1">
                  <span className="material-symbols-outlined text-[14px]">update</span>
                  Terakhir diperbarui: Hari ini
                </span>
                <button className="text-emerald-400 hover:text-emerald-300 font-label-sm text-label-sm transition-colors">Sync</button>
              </div>
            </div>
          ))}
          ...
          <button 
            ...
            className="bg-surface-container/20 backdrop-blur-[10px] border border-dashed border-outline-variant rounded-xl p-container-padding flex flex-col items-center justify-center min-h-[180px] group hover:bg-surface-container/40 hover:border-emerald-500/50 transition-all duration-300"
          >
            <div className="w-12 h-12 rounded-full bg-surface-container flex items-center justify-center mb-stack-md group-hover:bg-emerald-500/20 group-hover:text-emerald-400 transition-colors text-slate-400">
              <span className="material-symbols-outlined text-3xl">add</span>
            </div>
            <span className="font-headline-md text-headline-md text-slate-400 group-hover:text-emerald-400 transition-colors">Tambah Aset</span>
            <span className="font-label-sm text-label-sm text-slate-400/50 mt-1">Lacak investasi manual</span>
          </button>
        </div>
      </main>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-800 border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl relative animate-in fade-in zoom-in duration-200">
            ...
            <div className="space-y-4">
              <div>
                <label className="font-label-sm text-label-sm text-slate-400 block mb-2">Nama Aset (Sekuritas / Bank)</label>
                <input 
                  type="text" 
                  value={newAssetName}
                  onChange={(e) => setNewAssetName(e.target.value)}
                  placeholder="Contoh: Mandiri Sekuritas" 
                  className="w-full bg-surface-container-lowest border border-outline-variant text-slate-200 font-body-base focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-lg py-3 px-4 transition-all"
                />
              </div>
              
              <div>
                <label className="font-label-sm text-label-sm text-slate-400 block mb-2">Tipe Aset</label>
                <div className="grid grid-cols-2 gap-4">
                  <button 
                    onClick={() => setNewAssetType('Equity')}
                    className={`py-3 rounded-lg border font-label-sm flex items-center justify-center gap-2 transition-all ${newAssetType === 'Equity' ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400' : 'bg-surface-container border-outline-variant text-slate-400 hover:border-outline'}`}
                  >
                    <span className="material-symbols-outlined text-sm">trending_up</span> Investasi
                  </button>
                  <button 
                    onClick={() => setNewAssetType('Savings')}
                    className={`py-3 rounded-lg border font-label-sm flex items-center justify-center gap-2 transition-all ${newAssetType === 'Savings' ? 'bg-secondary/10 border-secondary text-secondary' : 'bg-surface-container border-outline-variant text-slate-400 hover:border-outline'}`}
                  >
            ...
            <div className="flex gap-3 mt-8 justify-end">
              <button 
                ...
                className="px-5 py-2.5 rounded-lg text-slate-400 hover:bg-surface-container hover:text-slate-200 font-label-sm transition-colors"
              >
                Batal
              </button>
              <button 
                ...
                className="px-5 py-2.5 rounded-lg bg-emerald-500 text-on-primary font-label-sm font-bold shadow-lg shadow-emerald-500/20 hover:bg-emerald-500/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                Simpan
              </button>
            </div>
      ...
      {deleteTargetId && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-800 border border-error/20 rounded-2xl p-6 w-full max-w-sm shadow-2xl relative animate-in fade-in zoom-in duration-200 text-center">
            ...
            <p className="font-body-base text-body-base text-slate-400 mb-6">
              Kartu aset ini dan riwayat alokasinya akan dihapus dari portofolio Anda.
            </p>
            <div className="flex gap-3 justify-center">
              <button 
                onClick={() => setDeleteTargetId(null)}
                className="px-5 py-2.5 rounded-lg border border-outline-variant text-slate-400 hover:bg-surface-container hover:text-slate-200 font-label-sm transition-colors flex-1"
              >
                Batal
              </button>
              <button 
